...
class BbcentralPipeline:

    # Database setup
    conn = sqlite3.connect('bcentral.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bcentral`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bcentral WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bcentral (date, title, link, content)"
